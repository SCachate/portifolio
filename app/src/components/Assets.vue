    ...
    <AsyncLoader :loading="loading" :error="!!error">
              ...
              <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-800/30">
                  <tr v-for="item in grupo.itens" :key="item.ativo" class="hover:bg-slate-800/20 transition-all group">
                    <td class="px-8 py-5">
                      <div class="font-bold text-white group-hover:text-emerald-400 transition-colors uppercase tracking-tight">{{ item.ativo || '---' }}</div>
      ...
      <template v-else-if="!loading && assets?.length === 0">
        <div class="flex flex-col items-center justify-center py-32 opacity-40">
          ...
          <p class="text-white font-bold tracking-[0.3em] uppercase text-xs">Nenhum ativo na carteira</p>
        </div>
      </template>
