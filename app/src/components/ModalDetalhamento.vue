          ...
          <div class="flex flex-1 overflow-hidden">
            <aside class="w-72 border-r border-slate-700 flex flex-col bg-slate-900/40">
              ...
              <div class="flex-1 overflow-y-auto custom-scrollbar">
                ...
                <button 
                  ...
                  :class="[
                    'w-full p-3 flex flex-col border-b border-slate-800/40 hover:bg-slate-800/60 transition-all text-left', 
            ...
            <main class="flex-1 flex flex-col bg-slate-900/10 overflow-hidden">
              <div v-if="assetSelecionado" class="flex-1 flex flex-col p-4 overflow-hidden">
                ...
                <div class="flex items-center justify-between gap-4 mb-4 bg-slate-800/20 p-4 rounded-xl border border-slate-800">
                  ...
                  <div class="flex-1 grid grid-cols-4 gap-4 px-4">
                    <div class="flex flex-col"><span class="text-[8px] text-slate-500 uppercase font-bold">Custódia</span><span class="text-sm text-white font-semibold">--</span></div>
                    <div class="flex flex-col"><span class="text-[8px] text-slate-500 uppercase font-bold">Preço Médio</span><span class="text-sm text-white font-semibold">--</span></div>
                    ...
                    <table class="w-full text-left text-[11px]">
                      ...
                      <tbody class="divide-y divide-slate-800/60 text-slate-300">
                        <tr v-for="i in 10" :key="i" class="hover:bg-white/5 transition-colors">
                          <td class="px-4 py-2 font-mono text-slate-400 opacity-50">--/--/----</td>
                          <td class="px-4 py-2"><span class="text-[9px] font-bold text-slate-600">SEM DADOS</span></td>
                          <td class="px-4 py-2 text-right">--</td>
                          <td class="px-4 py-2 text-right">R$ 0,00</td>
                          <td class="px-4 py-2 text-right font-bold text-white uppercase">R$ 0,00</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
              ...
              <div v-else class="h-full flex flex-col items-center justify-center text-slate-700">
                <p class="text-[10px] tracking-[0.2em] uppercase font-bold opacity-20">Selecione um ativo para detalhar</p>
              </div>
            </main>
          </div>
        </div>
...
const props = defineProps({
  modelValue: Boolean,
  classeSelecionada: String, // Label do dashboard (ex: "FIIs")
  tipo: String 
});
...
// --- FILTRO DE BUSCA (POR DESCRIÇÃO/NOME_ATIVO) ---
const assetsFiltrados = computed(() => {
  ...
  return lista.filter(a => {
    // Procura na descrição e no ticker (conforme nomes do SQL)
    const nome = a.nome_ativo ? a.nome_ativo.toLowerCase() : '';
    const ticker = a.Ticker ? a.Ticker.toLowerCase() : '';
...
const aoMudarClasseManual = (event) => {
  ...
  assetSelecionado.value = null; // Limpa seleção ao mudar de classe
