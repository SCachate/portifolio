          ...
          <header class="p-4 border-b border-slate-700 bg-slate-800/20 flex flex-wrap items-end justify-between gap-4">
            <div class="flex items-end gap-4">
              <div class="flex flex-col gap-1">
                ...
                <select :value="classeSelecionada" @change="$emit('update:classe', $event.target.value)" class="bg-slate-900 border border-slate-700 text-white font-bold rounded-lg p-1.5 px-3 text-sm outline-none focus:border-blue-500 transition-all">
            ...
            <aside class="w-64 border-r border-slate-700 flex flex-col bg-slate-900/40">
              ...
              <div class="flex-1 overflow-y-auto custom-scrollbar">
                <button v-for="asset in assetsFiltrados" :key="asset.id" @click="assetSelecionado = asset"
                  :class="['w-full p-3 flex flex-col border-b border-slate-800/40 hover:bg-slate-800/60 transition-all', assetSelecionado?.id === asset.id ? 'bg-blue-600/10 border-l-2 border-l-blue-500' : 'border-l-2 border-l-transparent text-left']">
            ...
            <main class="flex-1 flex flex-col bg-slate-900/10 overflow-hidden">
              <div v-if="assetSelecionado" class="flex-1 flex flex-col p-4 overflow-hidden">
                ...
                <div class="flex items-center justify-between gap-4 mb-4 bg-slate-800/20 p-3 rounded-xl border border-slate-800">
                  ...
                  <div class="flex-1 grid grid-cols-4 lg:grid-cols-7 gap-2">
                    <div class="flex flex-col">
                      <span class="text-[8px] text-slate-500 uppercase font-bold">Qtd</span>
                      <span class="text-xs text-white font-semibold">150</span>
                    </div>
                    <div class="flex flex-col">
                      <span class="text-[8px] text-slate-500 uppercase font-bold">P. Médio</span>
                      <span class="text-xs text-white font-semibold">R$ 45,20</span>
                    </div>
                    <div class="flex flex-col">
                      <span class="text-[8px] text-slate-500 uppercase font-bold">Investido</span>
                      <span class="text-xs text-white font-semibold">R$ 6.780,00</span>
                    </div>
                    <div class="flex flex-col">
                      <span class="text-[8px] text-slate-500 uppercase font-bold">P. Atual</span>
                      <span class="text-xs text-emerald-400 font-semibold">R$ 48,41</span>
                    </div>
                    <div class="flex flex-col">
                      <span class="text-[8px] text-slate-500 uppercase font-bold">V. Atual</span>
                      <span class="text-xs text-emerald-400 font-semibold">R$ 7.262,10</span>
                    </div>
                    <div class="flex flex-col col-span-2 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                      <span class="text-[8px] text-emerald-500 uppercase font-black">Resultado Período</span>
                      <span class="text-xs text-emerald-400 font-bold">R$ 482,10 (+7.1%)</span>
                    </div>
                  </div>
                </div>
                ...
                <div class="flex-1 bg-slate-900/40 border border-slate-800 rounded-xl overflow-hidden flex flex-col">
                  <div class="overflow-x-auto custom-scrollbar">
                    <table class="w-full text-left text-xs">
                      <thead class="bg-slate-800 text-slate-400 uppercase font-bold sticky top-0 z-10">
                        <tr>
                      ...
                      <tbody class="divide-y divide-slate-800/60 text-slate-300">
                        <tr v-for="i in 15" :key="i" class="hover:bg-white/5 transition-colors">
                          <td class="px-4 py-2.5">12/04/2026</td>
                          <td class="px-4 py-2.5">
                            <span class="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 rounded text-[9px] font-bold">COMPRA</span>
                          </td>
                          <td class="px-4 py-2.5 text-right font-mono">10</td>
                          <td class="px-4 py-2.5 text-right font-mono">R$ 45,00</td>
                          <td class="px-4 py-2.5 text-right font-mono text-white">R$ 450,00</td>
                          <td class="px-4 py-2.5 text-right font-mono text-emerald-400">+ R$ 34,10</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
              ...
              <div v-else class="h-full flex flex-col items-center justify-center text-slate-600">
                <p class="text-sm">Selecione um ativo para visualizar o relatório.</p>
              </div>
            </main>
          </div>
        </div>
